...
- [x] Since I know French (`fr`) reasonably well and German (`de`) passably so, target those two languages as second and third syntaxes. Should really be a copy of the `en-us` parser over and change just the keywords in the g4 parser grammar file.... emphasis on *should*. Oh heck, let's do Pig Latin (`en-pl`) as well, just for fun.

    **NOTE:** I've got `en-us`, `en-pl`, and `fr` parsers and tests finished.
