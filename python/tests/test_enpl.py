...
def run(code, local_vars = None) -> dict:
    ...
    # This little hack is necessary because ifway we assign to a function default,
    # and the default is a reference (not a value), we actually modify the default value!
    # In other words, ifway local_vars were defaulted to {} above, passing it through the
...
ifway a < 1 oday
    a = 1;
elseway ifway a < 2 oday
    a = 1;
elseway ifway a < 3 oday
    a = 1;
elseway ifway a < 10 oday
    a = 10;
elseway oday
...
def test_for() -> None:
    ...
    code = """
b = 0;
orfay a = 0 otay 3 oday
    b = b + 1;
endway
"""
    local_vars = run(code)
    assert local_vars['a'] == 2 # Upper endway is exclusive
    assert local_vars['b'] == 3
