    ...
    class Visitor(LinguisParserVisitor):
        ...
        def visitBlock(self, ctx:LinguisParser.BlockContext):
            ...
            for child in ctx.getChildren():
                retval = self.visit(child)
                self.logger.info(f"Visiting {child} got us a {retval}")
                statements.append(retval)
            return statements
        ...
        def visitStatement(self, ctx:LinguisParser.StatementContext):
            ...
            self.logger.info(f"Statement -> {retval}")
            return retval
        ...
        def visitIfStatement(self, ctx:LinguisParser.IfStatementContext):
            self.logger.info("visiting IfStatement")
            return self.visitChildren(ctx)
        ...
        def visitIdentifierExpression(self, ctx:LinguisParser.IdentifierExpressionContext):
            retval = ast.Name(ctx.getText(), ctx=ast.Load()) 
                # I think Load() or Store() will need to be flipped by another element
                # in the AST later once its clear if its a Load or Store operation?
